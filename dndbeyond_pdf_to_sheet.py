...
class CharacterSheetWriter:
    """
    Base class for writing character sheets.
    """
...
class CharacterSheetWriterGsheet(CharacterSheetWriter):
    ...
    # Mapping of character sheet fields to Google Sheet cell locations
    ...
    def write_to_sheet(self, worksheet_name: str, mapped_data: Dict[str, Any]) -> None:
        """
        ...
        for (
            field,
            cell,
        ) in CharacterSheetWriterGsheet.sheet_fields_to_sheet_cells_map.items():
...
class CharacterSheetReader:
    """
    Base class for reading character sheets.
    """


class CharacterSheetReaderDnDBeyond(CharacterSheetReader):
    """
    Class for reading character sheets from D&D Beyond.
    """

    # Each field is defined as a tuple of (page_index, anchor_point, line_index) where
    # anchor_point is a string to search for and line_index is the line offset
    ...
    @staticmethod
    def find_index_of_string_in_character_data(
        target: str, character_data: list
    ) -> tuple:
        """
    ...
    def load_character_data(self) -> dict:
        """
        Load character data from the PDF file.
        ...
        self.map_dnd_beyond_fields()
        self.cleanup_special_cases()
        self.parse_ability_saves()
        self.parse_skill_saves()
        return self.mapped_data

    def find_anchor_points(self) -> None:
        """
        ...
        :rtype: None
        """
        for anchor, _ in self.anchor_points_map.items():
            index = self.find_index_of_string_in_character_data(anchor, self.text_list)
            self.anchor_points_map[anchor] = index

    def map_dnd_beyond_fields(self) -> None:
        ...
        for (
            key,
            ix,
        ) in CharacterSheetReaderDnDBeyond.character_data_to_sheet_fields_map.items():
            print("Mapping field:", key, "Index:", ix)
            if ix is None:
                continue
            anchor = ix[1]
            relative_index = ix[2]
            absolute_index = self.anchor_points_map[anchor][1] + relative_index
            self.mapped_data[key] = self.text_list[ix[0]][absolute_index].strip()

    def cleanup_special_cases(self) -> None:
        """
        Cleanup special cases in the mapped character data.
        """
        self.mapped_data["subclass"] = ""
        self.mapped_data["class"] = self.mapped_data.get("class").split(" ")[0]
        self.mapped_data["level"] = self.mapped_data.get("level").split(" ")[-1]
        self.mapped_data["passive_abilities"] = (
            f"PER:{self.mapped_data.get('passive_perception')}, INS:{self.mapped_data.get('passive_insight')}, INV:{self.mapped_data.get('passive_investigation')}"
        )

    def parse_skill_saves(self) -> None:
        """
        ...
        anchor = self.anchor_points_map["Resistances"][1]

        # Set default proficiency flags to False
        for skill in skills:
            self.mapped_data[f"{skill}_proficiency"] = False

        # Fine first line of skill saves after anchor
        skill_saves_start = anchor
        while True:
            line = self.text_list[0][skill_saves_start].strip()
        ...
        while skills:
            ...
            line = self.text_list[0][current_position].strip()
            match line[0]:
                case "P":
                    self.mapped_data[proficiency] = True
                case "+" | "-":
                    self.mapped_data[bonus] = line
                    current_skill = None
                case _:
                    # Assume it's the modifier
                    self.mapped_data[mod] = line
            current_position += 1

    def parse_ability_saves(self) -> None:
        """
        ...
        anchor = CharacterSheetReaderDnDBeyond.character_data_to_sheet_fields_map[
            "strength_saving_throw"
        ][1]
        anchor_index = self.anchor_points_map[anchor][1]
        ability_saves_start = (
            anchor_index
            + CharacterSheetReaderDnDBeyond.character_data_to_sheet_fields_map[
                "strength_saving_throw"
            ][2]
        )
        # Initialize proficiency flags
        for ability in abilities:
            self.mapped_data[f"{ability}_proficiency"] = False
        current_ability = None
        ability_saves_end = self.anchor_points_map["Resistances"][1]
        # Iterate through ability saves
        for ability_index in range(ability_saves_start, ability_saves_end):
            if self.text_list[0][ability_index].strip() == "•":
                current_ability = abilities.pop(0)
                self.mapped_data[f"{current_ability}_proficiency"] = True
                continue
            elif not current_ability:
                current_ability = abilities.pop(0)
            self.mapped_data[f"{current_ability}_saving_throw"] = self.text_list[0][
                ability_index
            ].strip()
            current_ability = None
...
def main() -> None:
    ...
    reader = CharacterSheetReaderDnDBeyond(args.character_sheet)
    mapped_data = reader.load_character_data()
    print("Mapped Data:", mapped_data)

    # Write the mapped data to the Google Sheet
    writer = CharacterSheetWriterGsheet(args.google_sheet_name)
    writer.write_to_sheet("Page 1", mapped_data)
